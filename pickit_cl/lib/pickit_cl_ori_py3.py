...
def main(buildnumber, fourthree, buildtype):
    ...
    global fourthreeG
    global buildtypeG
    buildnumberG = buildnumber
    fourthreeG = fourthree
    ...
    resObj['build_class'] = soup.find(
        attrs={'class': 'classBadge'}).get('title')
    ...
    if soup.select('#kanai-weapon .db-title span'):
        resObj['kanai_weapon'] = soup.select(
            '#kanai-weapon .db-title span')[0].get_text()
        #print ( 'Kanai Weapon Slot | NAME={}'.format(resObj['kanai_weapon']))

    if soup.select('#kanai-armor .db-title span'):
        resObj['kanai_armor'] = soup.select(
            '#kanai-armor .db-title span')[0].get_text()
        #print ( 'Kanai Armor Slot | NAME={}'.format(resObj['kanai_armor']))

    if soup.select('#kanai-jewelry .db-title span'):
        resObj['kanai_jewelry'] = soup.select(
            '#kanai-jewelry .db-title span')[0].get_text()
        #print ( 'Kanai Jewelry Slot | NAME={}'.format(resObj['kanai_jewelry']))

    #print ( resObj)
    #print ( resObj['item_head'])
    pickitList = ';||||' + resObj['build_class'] + '-Build: ' + \
        resObj['build_name'] + ' Link: ' + resObj['build_url'] + ' ||||\n'
...
def getItemInfos(name, recursive=True):
    global buildnumberG
    ...
    try:
        if recursive:
            if name in synonyme_item_names:
                idex = name
                return getItemInfos(synonyme_item_names[index], False)
            if name in reverse_dict(synonyme_item_names):
                idex = name
                return getItemInfos(synonyme_item_names[index], False)
    except Exception:
        raise Exception('[getItemInfos] [bn: ' + buildnumberG + '] no match for "' +
                        name + '" in itemList. Even with not with synonymes considered.')
    raise Exception('[getItemInfos] [bn: ' + buildnumberG + '] no match for "' +
                    name + '" in itemList.')
    ...
    for item in itemList:
        ...
        if name['name'] == item['name']:
            try:
                ...
                raise KeyError(
                    '[generateCubeString()] [bn: ' + buildnumberG + '] type not found in item "' + str(item) + '" in itemList')
            try:
                pickitType = typeList[type]
            except KeyError:
                raise KeyError('[generateCubeString()] [bn: ' +
                               buildnumberG + ']' +
                               type + 'not found in typeList')
            return str(pickitType)
...
def getPickitStat(stat):
    ...
    for arr in statList:
        try:
            ...
            raise KeyError(
                '[getPickitStat()] [arr][1] [bn: ' + buildnumberG + ']: "' + str(arr) + '" not found in statList.')
        try:
            full = statList[arr][0]
        except KeyError:
            raise KeyError(
                '[getPickitStat()] [arr][0] [bn: ' + buildnumberG + ']: "' + str(arr) + '" not found in statList.')
        if (full == stat):
            return pickit
    ...
    for item in itemList:
        if name == item['name']:
            try:
                ...
                raise KeyError(
                    '[generateCubeString()] [bn: ' + buildnumberG + '] type not found in' + str(item) + 'of itemList')
            try:
                pickitType = typeList[type]
            except KeyError:
                raise KeyError('[generateCubeString()] [bn: '
