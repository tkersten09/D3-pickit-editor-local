...
RETRIABLE_EXCEPTIONS = [
    IOError, requests.exceptions.SSLError, urllib.error.URLError,
]
max_retries = 5
...
def getBattleNetItems():
    ...
    for cat in catArr:
        def func(): return collectBattleNetItems(cat)
        retriable_exceptions(func,
...
def parseBattleNetItems(body, type):
    ...
    descSel = soup.select(
        ".legendary .item-details-text a, .set .item-details .item-details-text a")
