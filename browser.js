...
onload = function() {
  ...
  const {ipcRenderer} = require('electron')
      ipcRenderer.on('update_buildnumbers', (event, class_name, new_selection) => {
        msg = class_name + new_selection
        new_selection_json = strMapToJson(new_selection)
        client.invoke("update", class_name, new_selection_json, (error, res) => {
          if(error) {
            console.error("Error when sending JSON msg" + msg + " to update: " + error)
          } else {
            console.log("Send update with JSON msg: " + msg)
          }
        })
    })
